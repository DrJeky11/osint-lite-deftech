...
async def summarize_posts(
    ...
    if not posts:
        return "No Bluesky posts found for the given query."

    formatted = "\n\n".join(
        f"[{i + 1}] @{post['author']['handle']}\n"
        f"Date: {post.get('created_at') or post.get('indexed_at') or 'Unknown'}\n"
        f"Likes: {post.get('likes', 0)} | Reposts: {post.get('reposts', 0)} | Replies: {post.get('replies', 0)}\n"
        f"{post.get('text') or ''}"
    ...
    prompt = (
        f"The user requested Bluesky posts on: {request_description}\n\n"
        f"{question_block}"
        f"Below are {len(posts)} Bluesky posts that matched the search. "
