...
def _recent_search_now() -> datetime:
    # X recent search docs note the default effective end time is slightly behind real time.
...
async def search_posts_raw(
    *,
    ...
    if not bearer_token:
        raise RuntimeError(
            "X_BEARER_TOKEN is not configured. Add it to scrapers/x/.env to enable X recent-search requests."
        )
